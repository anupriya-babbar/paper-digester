...
**Version 4.0 — Eval Calibration & Weighted Scoring**
...
| AI Model | claude-haiku-4-5 — summarizer + judge (calibrated, reference-anchored) |
...
### 0.8 Eval Calibration & Weighted Scoring (v4.0)

> **The v3 eval ran but disagreed with human judgment.** We built a golden dataset, measured the disagreement, diagnosed the cause, and calibrated the judge. We also moved from flat averaging to weighted scoring.

#### Golden Dataset — Human-in-the-Loop Calibration

We hand-scored 4 NLP/LLM papers (Graph Attention Networks, A Survey of Large Language Models, Large Language Models are Zero-Shot Reasoners, A Prompt Pattern Catalog) reading the FULL paper, not just the abstract. Each was scored Bad / Okay / Good on Faithfulness, Coverage, Mode Fidelity, and Teaching Quality. This dataset is the measurement instrument — it tells us whether the judge agrees with humans.

Human-first scoring (before running the LLM eval) keeps human judgment unbiased.

#### The Calibration Finding

| Dimension | Human | LLM (before) | Problem |
|---|---|---|---|
| Faithfulness | Good (4–4.5/5) | Bad (0–40%) | Judge far too strict |
| Coverage | Okay (2.5–3.5/5) | Good (85–100%) | Judge too generous |
| Mode Fidelity | Good (4/5) | Okay (50–75%) | Judge slightly strict |

Initial agreement: **0/12** across 4 papers × 3 dimensions.

#### Root Cause of Faithfulness Miscalibration

The judge received only the abstract as ground truth, then flagged every summary claim not present in the abstract as a violation — including correct details the summarizer pulled from the full paper. It confused "information beyond the abstract" (usually fine) with "contradicts or fabricates" (a real violation).

#### The Calibration Fix

The faithfulness prompt was rewritten to tell the judge that the summary was generated from the full paper, so details beyond the abstract are expected and good. It now flags ONLY direct contradictions, fabricated numbers, or misrepresentations. Simplifications and elaborations no longer reduce the score.

**Result after calibration:**

| Paper | Faithfulness Before | After | Human | Aligned |
|---|---|---|---|---|
| Graph Attention Networks | 40% | 79% | Good | ✓ |
| A Survey of LLMs | 20% | 92% | Good | ✓ |
| Zero-Shot Reasoners | 40% | 87% | Good | ✓ |
| Prompt Pattern Catalog | 0% | 77% | Good | ✓ |

Faithfulness agreement: **0/4 → 4/4.**

#### Coverage — Full-Text Proxy

The Coverage judge was given more context than the abstract alone: the stored summary fields (concept, findings, key_advantage, results, figures) act as a proxy for the full paper, since the summarizer already distilled the paper into them. The prompt also now checks four explicit coverage dimensions — main contribution, concrete results, mechanism, and scope.

#### Weighted Scoring (accuracy-first philosophy)

Overall score moved from flat average to weighted average. Rationale: in research, an inaccurate summary is more harmful than an incomplete one, so faithfulness carries the highest weight.

**Summary Eval weights:**
...
| Gap Novelty | 20% |
| Contradiction Reality | 15% |
| Citation Density | 10% |

**Null handling:** when a dimension is null (e.g. no abstract → faithfulness null), its weight is redistributed proportionally across the remaining non-null dimensions, so the overall isn't artificially depressed.

#### Chain Eval Fixes

Three bugs were resolved so all 5 chain dimensions now score:
- **paperIds casing** — chain stored `paperIds` (camelCase) but eval read `paper_ids`; now reads both.
- **Dual chip format** — synthesis used `[P1: 2017]` in evolution/agreements but `P1 [2017]` in contradictions; the chip regex now matches both, so citation grounding and contradiction reality fire correctly.
- These had previously caused Citation Grounding and Contradiction Reality to show N/A.

#### Search Fixes (supporting golden dataset creation)

- **Hybrid search** — exact title → title search → keyword fallback, so exact titles return the correct paper.
- **arXiv ID lookup** — pasting an ID like `1706.03762` fetches that exact paper, falling back to the arXiv API directly when OpenAlex lacks the link.

#### Critical Bug Fixed — Disappearing Summary Evals

A leftover cleanup `useEffect` (from the old Gemini eval format) deleted every new-format summary eval on page load — it matched on `results->scores IS NULL`, but the new format has no `scores` key, so every record matched. Removing this cleanup made summary evals persist across refreshes. Overall is now displayed as a score circle in the Overview tab, separated from dimension bars.

#### New / Changed Files (v4.0)

```
src/prompts/evalPrompts.js      — faithfulnessPrompt rewritten (calibration);
                                  coveragePrompt rewritten (four-dimension + full-text proxy)
src/hooks/useEval.js            — weighted scoring with null redistribution (summary + chain);
                                  dual-format chip handling; paperIds fallback
src/utils/freeChecks.js         — dual chip-format regex in extractCitationClaims +
                                  extractContradictionPairs; lengthSanity N/A reason
src/utils/evalStorage.js        — accumulate evals (no delete-before-insert)
src/components/DevDashboard.jsx  — removed destructive cleanup useEffect; Overall score circle;
                                  N/A labels with reasons; full-text proxy passed to eval
api/judge.js                    — dotenv config for local vercel dev
api/search.js                   — hybrid search + arXiv ID direct lookup
```
...
| Q&A on paper | Follow-up questions with paper context. Enter key submits. | P1 | Pending |
...
Overview tab aggregates with `computeOverviewStats()` — shows weighted overall as a score circle, per-dimension bars, and a needs-attention list (overall < 70). Evals accumulate across sessions (no delete-before-insert).

### 7.4 Weighted Scoring

Overall is a weighted average, not a flat mean — accuracy-first philosophy.

Summary: Faithfulness 40%, Coverage 35%, Mode Fidelity 15%, Free Checks 10%.
Chain: Citation Grounding 30%, Synthesis Quality 25%, Gap Novelty 20%, Contradiction Reality 15%, Citation Density 10%.

Null dimensions redistribute their weight proportionally across remaining dimensions so the overall isn't artificially depressed.

### 7.5 Golden Dataset & Judge Calibration

A 4-paper human-scored golden dataset (read against full papers, scored Bad/Okay/Good per dimension) serves as the calibration instrument. Initial human–LLM agreement on Faithfulness was 0/4 — the judge was over-strict because it only saw the abstract and flagged legitimate full-paper details as violations. After rewriting the faithfulness prompt to expect full-paper detail and flag only contradictions/fabrications, agreement rose to 4/4. The golden dataset is a measurement tool, not training data — it reveals miscalibration so judge rules can be rewritten and re-measured.

### 7.6 Capstone Eval Methodology Statement

> "We evaluate using a reference-anchored, calibrated LLM-as-judge system. Each summary dimension is grounded in the paper's abstract plus the distilled summary fields as a full-text proxy. We built a human-scored golden dataset, measured judge–human agreement, found the faithfulness judge was over-strict (0/4 agreement), diagnosed the abstract-only grounding as the cause, recalibrated the prompt, and restored agreement to 4/4. Overall scores use accuracy-first weighting with proportional redistribution for null dimensions. Score variance of ±10–15% across runs is expected due to LLM non-determinism; we report aggregates."
...
| TC-04 | Build demo chain (5 NN papers) → Run Chain Eval → all 5 dimensions score (no N/A) |
...
| TC-07 | Paste arXiv ID (e.g. 1706.03762) → exact paper returned |
| TC-08 | Cold user test — give to classmate, observe without helping |
| TC-09 | Run Summary Eval → refresh page → Overview retains scores (no cleanup deletion) |
| TC-10 | Run eval on paper with no abstract → Faithfulness/Coverage show N/A with reason label, weighted overall redistributes |
| TC-11 | Golden dataset — re-run eval on 4 papers → Faithfulness lands in Good band (75–92%), matching human scores |
...
*Paper Digester PRD v4.0 — Built with Claude Code | Eval Calibrated & Weighted, June 2026*
